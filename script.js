function getTime(numberFrames, frameRate, displayFormat) {
	var frame = numberFrames || 1036799,
	    fps = frameRate || 24,
	    format = displayFormat || 'frames',
	    time = Math.floor(frame/fps),
	    hours = Math.floor(time/3600),
	    minutes = Math.floor((time-hours*3600)/60),
	    seconds = time-(hours*3600+minutes*60),
	    frames = frame%fps,
	    milsecs = Math.floor(1000*(frame%fps)/fps);
	
	if(frames < 10 && fps < 120 || frames >= 10 && frames < 100 && fps >= 120) {
		frames = `0${frames}`;
	} else if(frames < 10 && fps >= 120) {
		frames = `00${frames}`;
	}
	if(milsecs < 10) {
		milsecs = `00${milsecs}`;
	} else if(milsecs >= 10 && milsecs < 100) {
		milsecs = `0${milsecs}`;
	} 
